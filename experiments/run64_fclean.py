...
OUT='/home/lee/lga/results64/'; log,_=L.mklog(OUT)
...
for s_ in range(2020,2026):
    ...
    gA=pd.Series(np.ones(prev.sum())).groupby(pid[prev]).size()
    ...
    F['p_ctm'][tgt]=r_as[tgt]-rate           # asof 오염 크기
FC=pd.DataFrame(F,index=R.index)
log('결측률 '+str(FC.isna().mean().round(4).to_dict()))
log(f"p_ctm  평균 {np.nanmean(FC.p_ctm):+.4f}  sd {np.nanstd(FC.p_ctm):.4f}  |>0.02| {np.nanmean(np.abs(FC.p_ctm)>0.02):.3f}")
