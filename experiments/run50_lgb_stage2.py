"""
50차 (49차 2단계) — LightGBM 재탐색 (블렌드 주력 축이 된 뒤 처음)
...
OUT='/home/lee/lga/results50/'; os.makedirs(OUT,exist_ok=True)
...
log(f'{el()} [{GRP}] v16 {X.shape[1]}피처 · leaves10 기준 · 스레드 {NTH}')

BASE=dict(n_estimators=1200, learning_rate=0.01, num_leaves=10, min_child_samples=6000,
...
GA=[('기준 leaves10', {}),
    ...
    ('n2400+mcs12000', dict(n_estimators=2400, learning_rate=0.005, min_child_samples=12000))]
GB=[('기준 leaves10', {}),
    ('colsample.3',   dict(colsample_bytree=0.3)),
    ('colsample.2',   dict(colsample_bytree=0.2)),
    ('colsample.15',  dict(colsample_bytree=0.15)),
    ('cs.2+mcs12000', dict(colsample_bytree=0.2, min_child_samples=12000)),
    ('lambda200',     dict(reg_lambda=200.)),
    ('lambda500',     dict(reg_lambda=500.)),
    ('lambda1000',    dict(reg_lambda=1000.)),
    ('subsample.5',   dict(subsample=0.5)),
    ('extra_trees',   dict(extra_trees=True)),
    ('mcs20000',      dict(min_child_samples=20000)),
    ('mcs40000',      dict(min_child_samples=40000)),
    ('mcs60000',      dict(min_child_samples=60000)),
    ('cs.2+mcs40000', dict(colsample_bytree=0.2, min_child_samples=40000))]
